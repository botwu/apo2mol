...
FOLLOW_BATCH = ('protein_element', 'ligand_element', 'ligand_bond_type', 'protein_translations')
...
class ApoHoloLigandData(Data):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    @staticmethod
    def from_apo_holo_ligand_dicts(apo_dict=None, holo_dict=None, ligand_dict=None, **kwargs):
        instance = ApoHoloLigandData(**kwargs)

        if apo_dict is not None:
            for key, item in apo_dict.items():
                instance['protein_' + key] = item

        if holo_dict is not None:
            for key, item in holo_dict.items():
                if key == 'pos':
                    instance['protein_' + key + '_holo'] = item
