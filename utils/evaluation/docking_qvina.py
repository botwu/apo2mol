...
class QVinaDockingTask(BaseDockingTask):
    ...
    def from_generated_mol(cls, ligand_rdmol, ligand_filename, protein_root='./data/crossdocked', **kwargs):
        ...
        if 'crossdock' in protein_root:
            ...
            with open(protein_path, 'r') as f:
                pdb_block = f.read()
        elif 'pdbbind' in protein_root:
            protein_file_name = os.path.basename(ligand_filename).replace('.sdf', '') + '_aligned_to_' + os.path.dirname(ligand_filename) + '.pdb'
            protein_fn = os.path.join(
                os.path.dirname(ligand_filename),
                protein_file_name
            )
